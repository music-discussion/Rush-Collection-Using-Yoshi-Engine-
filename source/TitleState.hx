	...
	*/
	function startIntro()
	{
		...
		if ((conf = ModSupport.modConfig[Settings.engineSettings.data.selectedMod]) != null && conf.intro != null) {
			...
			if (introConf.authors == null) introConf.authors = ['Discussions', 'Teu', 'Glitch', 'SMB'];
			if (introConf.present == null) introConf.present = 'present';
			if (introConf.assoc == null) introConf.assoc = ['In association', 'with'];
			if (introConf.newgrounds == null) introConf.newgrounds = 'Rush Collection';
			if (introConf.gameName == null) introConf.gameName = ['Friday Night Funkin\'', 'Rush Collection', 'Full Mod'];
		} else {
			introConf = {
				bpm: 102,
				authors: ['Discussions', 'Teu', 'Glitch', 'SMB'],
				present: 'present',
				assoc: ['In association', 'with'],
				newgrounds: 'Rush Collection',
				gameName: ['Friday Night Funkin\'', 'Rush Collection', 'Full Mod']
			};
		}
		...
		ngSpr = new FlxSprite(0, 800).loadGraphic(Paths.customizableImage('newgrounds_logo'));
		add(ngSpr);
		ngSpr.visible = false;
		ngSpr.setGraphicSize(Std.int(ngSpr.width * 0.8));
		ngSpr.updateHitbox();
		ngSpr.screenCenter(X);
		ngSpr.scale.set(0.25, 0.25);
