...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (!Settings.engineSettings.data.autoplayInFreeplay) {
			var t = new FlxText(0, 0, FlxG.width, '[Space] Listen to selected song | Selected Mod: ${ModSupport.getModName(Settings.engineSettings.data.selectedMod)} - Press [BACKSLASH] to switch.');
