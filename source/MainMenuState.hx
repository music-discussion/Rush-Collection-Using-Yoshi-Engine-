...
class MainMenuState extends MusicBeatState
{
	...
	{
		if (Settings.engineSettings.data.selectedMod != "Rush Collection Mod") Settings.engineSettings.data.selectedMod == "Rush Collection Mod";
		...
		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, 'Yoshi Engine v$yoshiEngineVer$buildVer - FNF v$fnfVer - Selected Mod: ${ModSupport.getModName(Settings.engineSettings.data.selectedMod)} (You cannot switch from Rush Collection using \\ (Backslash).)', 12);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxControls.justPressed.BACKSLASH) #if debug openSubState(new SwitchModSubstate()); #else trace('he tried'); #end
