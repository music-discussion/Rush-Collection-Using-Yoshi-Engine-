...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		GameOverSubstate.gameOverMusic = "Rush Collection:gameOver";
		...
		*/
		...
		fakeSongName = new FlxText(150, 150, 0, 'Rush E');
		fakeSongName.setFormat(Paths.font("closestIcouldFind.ttf"), Std.int(24), FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		fakeSongName.cameras = [camHUD];
		fakeSongName.antialiasing = true;
		fakeSongName.visible = false;
		fakeSongName.scale.set(1.5, 1.5);
		//fakeSongName.y -= fakeSongName.height / 2;
	...
	public var fakeSongName:FlxText = null;
	...
	function startSong():Void
	{
		...
		trace(CoolUtil.prettySong(SONG.song.toLowerCase()));
		fakeSongName.text = switch (CoolUtil.prettySong(SONG.song.toLowerCase())) //moved it here because I can lol
		{
			case "Rush Smb":
				"???: Rush SMB";
			default:
	...
	override function stepHit()
	{
		...
		switch (CoolUtil.prettySong(SONG.song.toLowerCase())) //moved it here because I can lol
		{
			case "Rush Smb":
				switch (curStep) 
				{
					case 1:
						trace("hey");
						fakeSongName.visible = true; fakeSongName.alpha = 0;
						fakeSongName.x = 75; fakeSongName.y = 55;
						fakeSongName.x += 20; trace(fakeSongName.x);
					case 313:
						fadeSong();
					case 2325:
						fadeSong(false);
					case 2470:
						var bgDim = new FlxSprite().makeGraphic(4000, 4000, FlxColor.BLACK);
						bgDim.scrollFactor.set(0);
						bgDim.screenCenter();
						add(bgDim);
						var no = new Alphabet(0, 200, 'Thanks for playing the mod!\n This is the secret song, Rush SMB.', true, false);
						no.cameras = [camHUD];
						no.screenCenter();
						no.scale.set(0.5, 0.5);
						add(no);
				}
			default:
				switch (curStep)
	...
	function fadeSong(fadeIn:Bool = true)
	{
		switch (fadeIn)
		{
			case true:
				trace(fakeSongName.x);
				FlxTween.tween(fakeSongName, {x: fakeSongName.x - 20, alpha: 1}, 1.4, {ease: FlxEase.cubeOut});
			case false:
				getReadyForNextFadeIn();
		}
	}

	function getReadyForNextFadeIn()
	{
		FlxTween.tween(fakeSongName, {alpha: 0}, 3.5, {ease: FlxEase.cubeOut});
		while (fakeSongName.alpha == 0)
		{
			fakeSongName.visible = true;
			fakeSongName.alpha = 0;
			fakeSongName.x += 20;
			trace(fakeSongName.x);
			break;
		}
